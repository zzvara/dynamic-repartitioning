				...
				for {
					...
					if (a < prevBalancePenalty) {
						val d2MigrationPenalty = d2Opt match {
							...
							case Some(d2) =>
								val d2Freq = currFreqsMap(d2)
								betaS(d2Freq) * indicator(prevP(d2) == j)
									-betaS(d2Freq) * indicator(prevP(d2) == i)
						}
						val plusMigrationPenalty =
							betaS(d1Freq) * indicator(prevP(d1) == i)
								-betaS(d1Freq) * indicator(prevP(d1) == j) + d2MigrationPenalty
