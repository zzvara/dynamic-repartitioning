...
case class PartitioningInfo(
	...
	override def toString: String = {
		s"PartitioningInfo [numberOfPartitions=$partitions, cut=$cut, sCut=$sCut, " +
			s"level=$level, heavyKeys = $heavyKeys]"
	}
}
	...
	def newInstance(globalHistogram: scala.collection.Seq[(Any, Double)], numPartitions: Int,
		...
		def computeCuts(i: Int): Unit = {
			if (i < startingCut && computedLevel <= sortedValues(i)) {
				...
				if (i < numPartitions - 1) computedLevel = remainder / (numPartitions - 1 - i)
				else computedLevel = 0.0d
				computedSCut += 1
				computeCuts(i + 1)
			}
		}
		...
		/**
			...
			*/
		val level = Math.max(0, (1.0d - sortedValues.take(actualSCut).sum) /
			(numPartitions - actualSCut))
		val actualCut = actualSCut + actualPCut
		PartitioningInfo(numPartitions, actualCut, actualSCut, level, globalHistogram)
	}
}
