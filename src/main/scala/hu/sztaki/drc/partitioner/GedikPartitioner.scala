...
/**
	* Partitioning function construction based on paper by Gedik et al. (2014): Partitioning Functions
	* for Stateful Data Parallelism in Stream Processing.
	*/
object GedikPartitioner {
	...
	def main(args: Array[String]): Unit = {
		...
		val initHasher: String => Int = consistentHasher.get
	...
	class ConsistentHasher[HashCode](partitions: Int,
		...
		override def size: Int = partitions
	...
	class GedikPartitioner(partitions: Int,
		...
		override def get(key: Any): Int = partitioner(key)
	...
	/**
		...
		* Measured frequencies of top elements in previous partitioner. Must be sorted in descending
		* order.
		* @param prevP
		...
		* @param utility
		* Utility function (U) to set the importance of balance penalty (a) vs migration penalty
		* (\gamma).
		* @tparam K
	...
	def constructPartitionerRedist[K](numPartitions: Int,
		...
		// H_c in paper
		val consistentHash: K => Int = consistentHasher.get
		...
		def balancePenaltyByExplicitHash(h: Map[K, Int]): Double = {
			...
			val inverseMap =
			(0 until numPartitions).map(p => (p, Iterable())).toMap ++ h.groupBy(_._2).mapValues(_.keys)
